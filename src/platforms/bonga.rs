...
pub fn get_playlist(username: &str) -> Result<(Option<String>, Option<String>)> {
    ...
    let hls = match json.get("localData").ok_or_else(o!())?.get("videoServerUrl").ok_or_else(o!())?.as_str() {
        Some(o) => o,
        None => return Ok((None, None)),
    };
    let performer_data = json.get("performerData").ok_or_else(o!())?;
    if !performer_data.get("isOnline").ok_or_else(o!())?.as_bool().ok_or_else(o!())? {
        return Ok((None, None));
    }
    if performer_data.get("isAway").ok_or_else(o!())?.as_bool().ok_or_else(o!())? {
        return Ok((None, None));
    }
    let playlist_url = format!(
        "https:{}/hls/stream_{}/playlist.m3u8",
        hls,
        performer_data.get("username").ok_or_else(o!())?.as_str().ok_or_else(o!())?
    );
...
pub fn parse_playlist(playlist: &mut stream::Playlist) -> Result<Vec<stream::Stream>> {
    ...
    for line in playlist.playlist.as_ref().ok_or_else(o!())?.lines() {
        ...
        streams.push(stream::Stream::new(&filename, &url, None, id, Platform::BONGA));
    }
    Ok(streams)
}
